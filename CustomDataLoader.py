...
class Flowers102(Dataset):
    def __init__(self, size=(224, 224), dataset_path='/media/bonilla/My Book/102flowers/train/*', augment=True):
        self.size = size
        self.transform = transforms.Compose([self.ToTensor(augment)])
        self.images_list = sum([glob.glob(os.path.join(p, '*')) for p in glob.glob(dataset_path)], [])
    ...
    def __getitem__(self, idx):
        ...
        y = np.array(int(self.images_list[idx].split(os.sep)[6]) - 1)
